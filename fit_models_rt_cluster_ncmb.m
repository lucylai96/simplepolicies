function [results] = fit_models_rt_cluster_ncmb(m)
%{
    Fit data to the models you specify.
%}

load lb_final.mat
load simdata_ncmb.mat
data = simdata_ncmb;
...
elseif contains(m, 'fixed')
    ...
    param(5) = struct('name','compress','lb',0.1,'ub',10,'logpdf',@(x) 0,'label','\beta_{reduced}');
    param(6) = struct('name','b1','lb',1,'ub',500,'logpdf',@(x) 0,'label','b1');
    param(7) = struct('name','b2','lb',1,'ub',500,'logpdf',@(x) 0,'label','b2');
...
elseif contains(m, 'value_pg')
  ...
  elseif contains(m, 'pg')
    param(1) = struct('name','beta0','lb',1,'ub',10,'logpdf',@(x) 0,'label','\beta0');
    param(2) = struct('name','lrate_theta','lb',minlr,'ub',maxlr,'logpdf',@(x) sum(log(betapdf(x,a,b))),'label','\alpha_{\theta}');
    param(3) = struct('name','lrate_V','lb',minlr,'ub',maxlr,'logpdf',@(x) sum(log(betapdf(x,a,b))),'label','\alpha_V');
    param(4) = struct('name','lrate_beta','lb',minlr,'ub',maxlr,'logpdf',@(x) sum(log(betapdf(x,a,b))),'label','\alpha_\beta');
    param(5) = struct('name','lrate_p','lb',minlr,'ub',maxlr,'logpdf',@(x) sum(log(betapdf(x,a,b))),'label','\alpha_p');
    param(6) = struct('name','compress','lb',0.1,'ub',10,'logpdf',@(x) 0,'label','\beta_{reduced}');
    param(7) = struct('name','b1','lb',1,'ub',500,'logpdf',@(x) 0,'label','b1');
    param(8) = struct('name','b2','lb',1,'ub',500,'logpdf',@(x) 0,'label','b2');
